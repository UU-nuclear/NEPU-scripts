...
# Create the matrices need for the systematic uncs
...
S0 <- sysCompHandler$map(orig_expDt, sysDt, ret.mat = TRUE)
...
P <- sysCompHandler$cov(sysDt, ret.mat = TRUE)
...
for(Reac in reaction_channels) {
	...
	curExpDt <- expDt[EXPID %in% ExpIds]
	curExpDt[, IDX := seq_len(.N)]

	expSel <- sysDt[,str_remove(EXPID,"EXPID-")] %in% ExpIds
	S <- sysCompHandler$map(curExpDt, sysDt, ret.mat = TRUE)[,expSel]
	X <- as(as.matrix(P[expSel,expSel]),"dgCMatrix")

	minE <- min(curExpDt$L1)
	maxE <- max(curExpDt$L1)

	grid_idcs <- which(energyGridrandomFiles>minE & energyGridrandomFiles<maxE)
	grid_idcs <- c(min(grid_idcs)-1,grid_idcs,max(grid_idcs)+1)
	...
	# calculate the reduced data, considering all uncertainties
	D <- Diagonal(x=curExpDt$UNC^2)
	cholZ <- makeCholZ(D, S, X)
	fact1 <- mult_xt_invCov_x(SensitivityMatrix, D, S, X, cholZ = cholZ)
	fact2 <- t(SensitivityMatrix) %*% mult_invCov_x(curExpDt$DATA, D, S, X, cholZ = cholZ)
	...
	for(i in seq_len(100)) {
		curExpDt[,NEWUNC:=sqrt(sum(RESIDUAL^2)/(.N-1)),by=BIN_NUM]

...
rawUncDt <- getAvailableUncertainties(expDt, subents, dataref.col = "DATA")
# -----------------------------------------------------------------
# CHANGED: apply floor on auto-added systematic uncertainties.
#
# getAutoUncertainty adds a sys-rel component with UNC=0.1 (10%)
# when no systematic is reported in EXFOR (INFO=="auto-added").
# We lower this floor to autoSysUncFloor (e.g. 0.05 = 5%).
#
# We only touch rows that are:
#   1. systematic (sys-rel)
#   2. auto-added by the package (INFO == "auto-added")
#   3. currently at the hardcoded 0.1 value
# Explicitly reported EXFOR sys uncs are left completely unchanged.
# -----------------------------------------------------------------
rawUncDt[ERRTYPE == "sys-rel" &
         INFO == "auto-added" &
         UNC == 0.1,
         UNC := autoSysUncFloor]
 
cat("Systematic uncertainty floor applied:\n")
cat("  auto-added sys-rel set to", autoSysUncFloor*100, "%",
    "(was hardcoded 10%)\n")
...
expDt[, UNC := pmax(UNC, autoStatUncFloor * DATA, na.rm = TRUE)]

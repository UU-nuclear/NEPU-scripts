...
# dE <- 10
# Elow <- energyGridForParams[length(energyGridForParams)] + dE
# Eup <- energyGridrandomFiles[length(energyGridrandomFiles)]
# energyGridForParams_extension <- seq(Elow,Eup+dE-Eup%%dE,by=dE)

# take the energy-grid for the parameter extension to be the 
# energy grid for the cross-section
#energyGridForParams_extension <- energyGridrandomFiles[energyGridrandomFiles>energyGridForParams[length(energyGridForParams)]]
# keep only every third entry
#energyGridForParams_extension <- energyGridForParams_extension[seq(3,length(energyGridForParams_extension),by=3)]

# test 3: an energy grid with only 1 point at 200 MeV
energyGridForParams_extension <- c(200)

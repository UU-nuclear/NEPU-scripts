...
# Testing to let all energy dependent parameters be free if at least one is sensitive to data.
# this is otherwise exactly the same as gp-prior-before-lm-try2, with confif file
# config_gp_prior_before_lm.R
...
energyGridForParams <- c(0,3,seq(5,31,by=2))
...
outdataPath <- file.path(workdir, "/outdata-gp-prior-before-lm-all-endep-free")

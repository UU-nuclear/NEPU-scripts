...
refInpList <- c(refInpHeader,adjParList)
...
for(i in 1:nrow(par_unc)) {
    ...
    if(is.na(particle)) {
        ...
        refParamDt[grepl(name,refParamDt$PARNAME) & stri_sub(refParamDt$PARNAME,-1)==particle ,PARUNC := unc]
    }
}

# the uncertainy should be smaller than the parameter transformation
# using the error-function parameter transformation, the maximum unc
# is equal to the delta parameter of the transform. This corresponds to
# a uninformed prior (uniform distribution) on the allowed range
refParamDt[ADJUSTABLE==TRUE,PARUNC:=pmin(PARUNC,unlist(PARVAL)-PARMIN)]

...
# DESCRIPTION OF STEP
#
# This script is just a test to see what the effect 
# of systematic uncertainties are. We simply remove
# all of them and keep only the statistical errors.
#
# in order to not complicate the processing of data
# to remove systematics I just set them to very small
# values 1E-06
#
...
for (curReac in unique(expDt$REAC)) {
    ...
    lowerLims <- rep(1E-06,length(curSysDt[ADJUSTABLE == TRUE, UNC]))
    upperLims <- rep(2E-06,length(curSysDt[ADJUSTABLE == TRUE, UNC]))

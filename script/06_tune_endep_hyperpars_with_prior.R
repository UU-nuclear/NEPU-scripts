...
for (curParname in parnames_endep) {
    gpHandler$addGP(curParname, curSysDt[EXPID==curParname,UNC][1], 50, 1e-4)
}
...
lowerLims[gpDt$ADJUSTABLE & gpDt$PARNAME == "sigma"] <- 0.25*tmp$UNC
lowerLims[gpDt$ADJUSTABLE & gpDt$PARNAME == "len"] <- 3
upperLims[gpDt$ADJUSTABLE & gpDt$PARNAME == "sigma"] <- 2*tmp$UNC
...
priorExpectation[gpDt$ADJUSTABLE & gpDt$PARNAME == "sigma"] <- 0.5*tmp$UNC
priorExpectation[gpDt$ADJUSTABLE & gpDt$PARNAME == "len"] <- 50

priorUnc <- rep(NA_real_, nrow(gpDt))
priorUnc[gpDt$ADJUSTABLE & gpDt$PARNAME == "sigma"] <- 0.5*tmp$UNC
...
#########################
min_sigma <- max(0.1*tmp$UNC)
max_sigma <- min(2*tmp$UNC)
min_length <- 6
max_length <- 100
...
best_result <- results[,unlist(results["value",]) == max(unlist(results["value",]))][,1]

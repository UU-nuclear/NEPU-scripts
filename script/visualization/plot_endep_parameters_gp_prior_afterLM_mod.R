...
ggp1 <- ggp1 + theme_bw()
ggp1 <- ggp1 + scale_x_continuous(breaks=seq(0,200,50),minor_breaks=seq(0,200,10))
...
# ----- New plot with dynamic y-range per EXPID -----
# Compute dynamic y-limits
plotDt_adjustables[, YMIN := DATAMIN - 0.05*abs(DATAMIN)]
plotDt_adjustables[, YMAX := DATAMAX + 0.05*abs(DATAMAX)]

ggp1_dynamic <- ggplot(data=plotDt_adjustables[ERRTYPE=='talyspar_endep']) +
    theme_bw() +
    scale_x_continuous(breaks=seq(0,200,50), minor_breaks=seq(0,200,10)) +
    theme(text = element_text(size=10)) +
    xlab('energy') + ylab('parameter value relative to default') +
    ...
    facet_wrap(~ EXPID, ncol=4, scales="free_y")  # free y-axis per EXPID
ggp1 <- ggp1 + theme(
  axis.text.y = element_text(size = 6, lineheight = 0.9),
  strip.text = element_text(size = 6, lineheight = 0.9)
)
# Save dynamic y-range plot
filepath_dynamic <- file.path(plotPath, 'endep_parameters_with_gp_obs_dynamic_yrange.png')
ggsave(filepath_dynamic, ggp1_dynamic, width = 29.7, height = 21.0, units = "cm", dpi = 300)

...
apptainer exec --bind $BASE_DIR --bind $PROJ_DIR $SIF_FILE Rscript --vanilla $SCRIPT_DIR/03_extract_experimental_uncertainties_alt.R $CONFIG_FILE

apptainer exec --bind $BASE_DIR --bind $PROJ_DIR $SIF_FILE Rscript --vanilla $SCRIPT_DIR/04_correct_stat_unc.R $CONFIG_FILE

apptainer exec --bind $BASE_DIR --bind $PROJ_DIR $SIF_FILE Rscript --vanilla $SCRIPT_DIR/04_tune_experimental_uncertainties.R $CONFIG_FILE
